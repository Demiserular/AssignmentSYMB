...
// Professional banking-grade deposit form
const API = import.meta.env.VITE_API_URL || 'http://localhost:4000/api'

export default function Deposit({ onSuccess }) {
  ...
  async function submit(e) {
    ...
    const res = await fetch(`${API}/accounts/${form.accountNo}/deposit`, { 
      method: 'POST', 
    ...
    if (data.ok) {
      setMessage({ type: 'success', text: `Amount deposited successfully! New balance: ₹${data.account.balance.toFixed(2)}` })
      onSuccess()
      setTimeout(() => {
  ...
  return (
    <div className="form-card">
      <div className="form-header">
        <h2>Deposit Funds</h2>
        <p className="form-subtitle">Add money to your account</p>
      </div>
